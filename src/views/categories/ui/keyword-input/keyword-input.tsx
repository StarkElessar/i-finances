...
export function KeywordInput(props: KeywordInputProps) {
    ...
    const commitKeywords = (rawKeywords: readonly string[]) => {
        ...
        setError(hasRejectedKeyword ? `Ключевое слово не длиннее ${KEYWORD_MAX_LENGTH} символов` : undefined);
    ...
    return (
        <div class={cn(css.root, props.class)}>
            ...
            <Show when={error() || props.hint}>
                <div class={cn(css.message, error() && css.messageError)}>
                    {error() ?? props.hint}
                </div>
            </Show>
        </div>
