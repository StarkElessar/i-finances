import css from './contact-summary-dialog.module.scss';
...
/**
 * Controlled dialog that lists contact operations for a selected period.
 */
export type ContactSummaryDialogProps = {
...
/**
 * Dialog with period controls and a chronological list of contact operations.
 */
export function ContactSummaryDialog(props: ContactSummaryDialogProps) {
	...
	return (
		...
		>
			<Dialog.Content class={css.dialog}>
				<Dialog.Header closeLabel='Закрыть сводку контакта'>
					<Dialog.Kicker>Сводка</Dialog.Kicker>
					<Dialog.Title>{props.contact?.name ?? 'Контакт'}</Dialog.Title>
					<Dialog.Description>
						Операции контакта за выбранный период
					</Dialog.Description>
				</Dialog.Header>
...
/**
 * Renders one operation row inside the contact summary list.
 */
function ContactOperationRow(props: ContactOperationRowProps) {
		...
		<div class={css.row}>
			<div class={css.rowMain}>
				<div class={css.rowMeta}>
					...
					<Show when={props.operation.categoryName}>
						<span class={css.category}>{props.operation.categoryName}</span>
					</Show>
				</div>
