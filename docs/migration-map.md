...
| `src/views/sign-in/api` | `packages/contracts` + `apps/web/src/features/auth` | partial | Password sign-in/session client and UI migrated; passkey UI remains pending with WebAuthn. |
...
| `src/server/auth` | `apps/api/src/modules/auth` + `apps/api/src/http/auth-controller.ts` | partial | Password sign-in, Argon2 verification, rate limiting, strict origin guard, cookie sessions, and revoke flow migrated; WebAuthn remains. |
