...
describe('contact selectors', () => {
	...
	it('sorts contacts by monthly spent descending, then by name', () => {
		const unsorted = [
			createContact('zero', { name: 'Яков' }),
			createContact('mid', { name: 'Борис' }),
			createContact('top', { name: 'Анна' }),
			createContact('tie-b', { name: 'Виктор' }),
			createContact('tie-a', { name: 'Артём' })
		];

		expect(sortContactsByMonthlySpent(unsorted, {
			mid: 5_000,
			'tie-a': 10_000,
			'tie-b': 10_000,
			top: 20_000
		}).map((contact) => contact.id)).toEqual([
			'top',
			'tie-a',
			'tie-b',
			'mid',
